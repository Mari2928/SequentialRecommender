...
def sample_function(args, user_train, usernum, itemnum, batch_size, maxlen, relation_matrix, result_queue, SEED):
    def sample(user):

        ### FutureAllItem or FutureDenseAll ###################
        N = args.num_items # window size
        S = args.rand_neg  # number of random negatives
        seq = np.zeros([maxlen], dtype=np.int32)
        time_seq = np.zeros([maxlen], dtype=np.int32)
        pos = np.zeros([maxlen, N], dtype=np.int32)
        neg = np.zeros([maxlen, S], dtype=np.int32)
        nxts= [user_train[user][-1][0] for _ in range(N)]
        ...
        #     time_seq[idx] = i[1]
        #     pos[idx] = nxts
        #     if nxts[0] != 0: neg[idx] = random_neq(1, itemnum + 1, ts)             
        #     nxt = i[0]
        #     idx -= 1
        #     if idx == -1: break
        ########################################################
...
class WarpSampler(object):
    def __init__(self, args, User, usernum, itemnum, relation_matrix, batch_size=64, maxlen=10,n_workers=1):
        self.result_queue = Queue(maxsize=n_workers * 10)
        self.processors = []
        for i in range(n_workers):
            self.processors.append(
                Process(target=sample_function, args=(args,
                                                      User,
                                                      usernum,
                                                      itemnum,
                                                      batch_size,
...
def data_partition2(fname, args):
    usernum = 0
    itemnum = 0
    User = defaultdict(list)
    user_train = {}
...
def evaluate_next(model, dataset, args):
    ...
    for u in users:
        ...
        train[u] += valid[u]  
        #print('train',train[u],'valid',valid[u] )
        ...
        for i in range(0, len(test[u])):  
          ...
          prediction = -model.predict(*[np.array(l) for l in [[u], [seq], [time_matrix],item_idx]])
          pred = prediction[0].argsort().argsort()
          ranks.append(pred[0].item())  
          # add predicted item to seq
          train[u].append([item_idx[(pred == 0).nonzero(as_tuple=True)[0]], test[u][i][1]])
